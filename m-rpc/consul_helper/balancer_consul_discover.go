...
// 服务发现
func NewConsulResolver(consulAddress, consulPort string, service string, myService string, option ...Option) (*ConsulResolver, error) {
	...
	r := &ConsulResolver{
		...
		cpuUsage:     50,
		serviceRatio: 3,  //default
		cpuThreshold: 60, //default
	}
...
type Option func(*ConsulResolver)

func SetCpuThreshold(cpuThreshold float64) Option {
	return func(resolver *ConsulResolver) {
		resolver.cpuThreshold = cpuThreshold
	}
}

func SetServiceRatio(serviceRatio float64) Option {
	return func(resolver *ConsulResolver) {
		resolver.serviceRatio = serviceRatio
	}
}
...
func SetInterval(interval int) Option {
	return func(resolver *ConsulResolver) {
		resolver.interval = time.Duration(interval)
	}
}
